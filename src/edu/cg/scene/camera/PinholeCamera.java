...
public class PinholeCamera {
	...
	public Point transform(int x, int y) {
		...
		double yDistance = (y - (resY / 2));
		double xDistance = (x - (resX / 2));
		Vec upDistance = upVec.mult(ratio *  yDistance);
		Vec rightDistance = rightVec.mult(ratio * xDistance);

		return centerPoint.add(upDistance.neg()).add(rightDistance);
	}
